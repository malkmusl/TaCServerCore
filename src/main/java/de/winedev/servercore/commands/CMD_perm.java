  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    if (args.length == 4) { // chacking the args langh == 4 to run the main part of the command
      if (sender instanceof Player) { // chacking if it player or console
        ...
        if (p.hasPermission("servercore.perm")
            || p.hasPermission("servercore.*")
            || p.isOp()) { // chacking the permissions to run the command -> PlayerSide

          if (args[0].equalsIgnoreCase(
              "add")) { // chacking for add/remove/list this is add  -> PlayerSide

            if (args[1].equalsIgnoreCase(
                "user")) { // starting the user part of the command -> PlayerSide
              ...
              if (Bukkit.getOnlinePlayers().contains(target)) {
                ...
                if (!target.hasPermission(args[3].toString())) {
                  if (!userPerm.contains(args[3])) {
                    ...
                    p.sendMessage(
                        "§3Du hast dem Spieler » §e"
                            + PlayerName
                            + "§3 die Berechtigung » §e"
                            + args[3]
                            + "§3 gegeben");
                  } else {
                    p.sendMessage(
              ...
              // add userperm command end -> PlayerSide
                ...
                "group")) { // starting the group part of the command -> PlayerSide
              FileManager.load(groups);
              if (groups.getConfig().contains(args[2].toLowerCase(Locale.ROOT).toString())) {
                List groupsPerm =
                    groups
                        .getConfig()
                        .getList(args[2].toLowerCase(Locale.ROOT).toString() + ".permissions");
                if (!groupsPerm.contains(args[3])) {
                  groupsPerm.add(args[3].toString());
                  groups
                      .getConfig()
                      .set(args[2].toLowerCase(Locale.ROOT).toString() + "permissions", groupsPerm);
                  FileManager.save(groups);
                  p.sendMessage(
                      "§3Du hast der Gruppe » §e"
                          + args[2].toUpperCase(Locale.ROOT).toString()
                          + "§3 die Berechtigung » §e"
                          + args[3]
                          + "§3 gegeben");
                } else {
            ...
            // by using /perm <add/remove/list> <user/group> <target> [perm]
              ...
              "remove")) { // chacking for add/remove/list this is remove  -> PlayerSide
            if (args[1].equalsIgnoreCase(
                "user")) { // starting the user part of the command -> PlayerSide
        ...
        // end of the PlayerSide Command the Console side works near the same with out chacking the
        // permissions and use CommandSender instad of p(Player)

        // by using /perm <add/remove/list> <user/group> <target> [perm]

      } else { // begin of the console side command please read the annotations on top if you have
               // questions
        if (args[0].equalsIgnoreCase("add")) {
          ...
          Bukkit.getConsoleSender()
              .sendMessage("§cUsage: /perm <add/remove> <user/group> <target> <perm>");
        }
      }
      ...
      if (args[0].equalsIgnoreCase("list")) {
        ...
        Bukkit.getConsoleSender()
            .sendMessage("§cUsage: /perm <add/remove/list> <user/group> <target> <perm>");
      }
    } else {
